...
namespace ProgrammingAssignment2
{
    ...
    class Program
    {
        ...
        static void Code ()
        {
            ...
            float dist_Sq = (float)Math.Pow(delta_X, 2) + (float)Math.Pow(delta_Y, 2);

            float dist = (float)Math.Sqrt(dist_Sq);

            float arcTangent2 = (float)Math.Atan2(delta_Y, delta_X);

            float angleInDegrees = arcTangent2 * (180F / (float)Math.PI);

            Console.WriteLine(dist + " " + angleInDegrees);
        }
    }
}